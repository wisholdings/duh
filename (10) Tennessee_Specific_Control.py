...
DATE_COL = "date"
REGION = "tennessee"
REGION_UPPER = "TENNESSEE"

# --- TENNESSEEDataProcessor Class Definition ---
class TENNESSEEDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the TENNESSEEDataProcessor using Parquet files.
    ...
    def fill_TENNESSEE_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill TENNESSEE-specific data requirements:
        1. TENNESSEE_BAT_MW, TENNESSEE_SNB_MW: zeros until 2025-03-17 22:00
        2. TENNESSEE_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying TENNESSEE-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-01-14 22:00:00')

        # *** MODIFIED: Removed TENNESSEE_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['TENNESSEE_BAT_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'TENNESSEE_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for TENNESSEE_GEO_MW ***

        print("TENNESSEE-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the TENNESSEE data processing."""
        print(f"\n--- Processing TENNESSEE Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply TENNESSEE-specific data filling
            df_filled = self.fill_TENNESSEE_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting TENNESSEE-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if TENNESSEE is in the regions config
    if REGION not in regions:
        print(f"TENNESSEE not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing TENNESSEE region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[TENNESSEEDataProcessor] = None

    try:
        # Instantiate the TENNESSEE-specific processor
        processor = TENNESSEEDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"TENNESSEE (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"TENNESSEE (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- TENNESSEE Data Processing Complete ---")
    print("="*60)