...
TEXAS_COLUMN = "TEXAS_INTERCHANGE_MW"
DATE_COL = "date"
...
# --- TexasInterchangeMerger Class Definition ---
class TexasInterchangeMerger:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the TexasInterchangeMerger.
        ...
        self.Texas_column = TEXAS_COLUMN
    ...
    def load_existing_parquet(self) -> pd.DataFrame:
        """Load the existing Texas_complete_finished.parquet file."""
    ...
    def download_Texas_interchange(self) -> pd.DataFrame:
        """Download Texas interchange data from the database with date filtering."""
        if self.engine is None:
            print("❌ No database connection available.")
            return pd.DataFrame()

        try:
            print(f"Downloading Texas interchange data from {self.table_name}...")
            print(f"Filtering data from {self.start_date_filter} onwards...")
            
            # SQL query to get Texas data with date filter
            query = f"""
            SELECT 
                DATETIME,
                {self.Texas_column}
            FROM {self.table_name}
            WHERE {self.Texas_column} IS NOT NULL
            ...
            if df.empty:
                print("⚠️  No Texas interchange data found in the database for the specified date range.")
                return df
            ...
            if not df[self.Texas_column].empty:
                stats = df[self.Texas_column].describe()
                ...
                non_null_count = df[self.Texas_column].notna().sum()
    ...
    def merge_data(self, parquet_df: pd.DataFrame, interchange_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if interchange_df.empty:
            ...
            parquet_df[self.Texas_column] = None
            return parquet_df
        ...
        try:
            ...
            interchange_col_stats = merged_df[self.Texas_column].describe()
            non_null_interchange = merged_df[self.Texas_column].notna().sum()
    ...
    def run(self):
        """Execute the Texas interchange data merge process."""
        print(f"\n--- Starting Texas Interchange Data Merge ---")
        ...
        try:
            ...
            # 3. Download Texas Interchange Data
            interchange_df = self.download_Texas_interchange()
            ...
            if success:
                print(f"\n✅ Texas interchange data merge completed successfully!")
            ...
            for df_var in ['parquet_df', 'interchange_df', 'merged_df']:
                if df_var in locals() and not locals()[df_var].empty:
                    del locals()[df_var]
            gc.collect()
            print(f"\n--- Texas Interchange Merge Process Finished ---")

# --- Main Execution Block ---
if __name__ == "__main__":
    print("===== Starting Texas Interchange Data Merge =====")
    print(f"Input Parquet: Texas_complete_finished.parquet")
    print(f"Database table: {TABLE_NAME}")
    print(f"Texas column: {TEXAS_COLUMN}")
    print(f"Date filter: From {START_DATE_FILTER} onwards")
    print(f"Output Parquet: Texas_complete_with_interchange.parquet")
    
    merger = None

    try:
        # Instantiate the merger
        merger = TexasInterchangeMerger(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the merge process
        merger.run()

    except ValueError as e:
        print(f"Texas Interchange Merger (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Texas Interchange Merger (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Texas Interchange Data Merge Complete ---")
    print("="*60)