...
# --- CentralBackfillCleaner Class Definition ---
class CentralBackfillCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CentralBackfillCleaner.
        ...
        self.oth_columns = ["CENTRAL_OTH_RATIO", "CENTRAL_OTH_MW"]
    ...
    def clean_oth_data_after_reference(self, df: pd.DataFrame) -> pd.DataFrame:
        """Clean CENTRAL_OTH_RATIO and CENTRAL_OTH_MW data after reference date."""
        if df is None or df.empty:
    ...
    def run(self):
        """Execute the Central backfill and cleaning process."""
        print(f"\n--- Starting Central Backfill and OTH Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                print(f"\n--- Central backfill and cleaning completed successfully ---")
            ...
            for df_var in ['df', 'df_backfilled', 'df_cleaned']:
                if df_var in locals() and locals()[df_var] is not None:
                    del locals()[df_var]
            gc.collect()
            print(f"\n--- Central Backfill Cleaner finished ---")

# --- Main Execution Block ---
if __name__ == "__main__":
    print("===== Starting Central Backfill and OTH Cleaning Process =====")
    ...
    print(f"Output file: central_complete_backfilled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[CentralBackfillCleaner] = None

    try:
        # Instantiate the cleaner
        cleaner = CentralBackfillCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Central Backfill Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Central Backfill Cleaner (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Central Backfill and OTH Cleaning Complete ---")
    print("="*60)