...
# --- CentralDataWrangler Class Definition ---
class CentralDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CentralDataWrangler.
        ...
        self.mw_combinations = {
            "CENTRAL_OTH_MW": ["CENTRAL_OIL_MW", "CENTRAL_OTH_MW"],
        }
        
        self.capacity_combinations = {
            "Central_Total_Capacity_Other": [
                "Central_Total_Capacity_Other", 
                "Central_Total_Capacity_Oil", 
                "Central_Total_Capacity_Batteries",
                "Central_Total_Capacity_Solar_Batteries",
                "Central_Total_Capacity_Geothermal",
                "Central_Total_Capacity_Biomass",
            ],
                    "Central_Total_Capacity_Hydroelectric": [
                "Central_Total_Capacity_Hydroelectric", 
                "Central_Total_Capacity_Pumped_Storage", 
            ]
        }
    ...
    def run(self):
        """Execute the Central data wrangling process."""
        print(f"\n--- Starting Central Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. CENTRAL_OTH_MW = CENTRAL_OIL_MW + CENTRAL_OTH_MW + CENTRAL_GEO_MW")
                print(f"  2. CENTRAL_BATTERIES_MW = CENTRAL_BAT_MW + CENTRAL_PS_MW + CENTRAL_SNB_MW")
                print(f"  3. CENTRAL_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Central Data Wrangling Process =====")
    print(f"Input file: central_complete_with_interchange.parquet")
    print(f"Output file: central_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[CentralDataWrangler] = None

    try:
        # Instantiate the Central data wrangler
        wrangler = CentralDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Central Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Central Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Central Data Wrangling Process Complete ---")
    print("="*60)