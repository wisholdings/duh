...
# --- SoutheastDataCleaner Class Definition ---
class SoutheastDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the SoutheastDataCleaner.
    ...
    def forward_fill_non_Southeast_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'SOUTHEAST'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Southeast columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'SOUTHEAST' (excluding the date column)
        non_Southeast_cols = [col for col in df_out.columns 
                              if not col.startswith('SOUTHEAST') and col != self.date_col]
        
        print(f"Found {len(non_Southeast_cols)} non-Southeast columns to process:")
        for col in non_Southeast_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Southeast_cols) > 10:
            print(f"  ... and {len(non_Southeast_cols) - 10} more columns")

        if not non_Southeast_cols:
            print("  No non-Southeast columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Southeast column
        for col in non_Southeast_cols:
    ...
    def run(self):
        """Execute the Southeast data cleaning process."""
        print(f"\n--- Starting Southeast Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Southeast columns
            df_filled = self.forward_fill_non_Southeast_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Southeast Data Cleaning Process =====")
    print(f"Input file: Southeast_complete_finished.parquet")
    print(f"Output file: Southeast_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[SoutheastDataCleaner] = None

    try:
        # Instantiate the Southeast data cleaner
        cleaner = SoutheastDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Southeast Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Southeast Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Southeast Data Cleaning Process Complete ---")
    print("="*60)