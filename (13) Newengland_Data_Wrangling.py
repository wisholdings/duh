...
# --- NewenglandDataWrangler Class Definition ---
class NewenglandDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NewenglandDataWrangler.
        ...
        self.mw_combinations = {
            "NEWENGLAND_OTH_MW": ["NEWENGLAND_OIL_MW", "NEWENGLAND_OTH_MW"],
            "NEWENGLAND_BAT_MW": ["NEWENGLAND_SNB_MW","NEWENGLAND_BAT_MW"]
        }
        
        self.capacity_combinations = {
            "Newengland_Total_Capacity_Other": [
                "Newengland_Total_Capacity_Other", 
                "Newengland_Total_Capacity_Oil", 
                "Newengland_Total_Capacity_Biomass"

            ],
                "Newengland_Total_Capacity_Batteries": [
                "Newengland_Total_Capacity_Solar_Batteries", 
                "Newengland_Total_Capacity_Batteries", 
    ...
    def run(self):
        """Execute the Newengland data wrangling process."""
        print(f"\n--- Starting Newengland Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. NEWENGLAND_OTH_MW = NEWENGLAND_OIL_MW + NEWENGLAND_OTH_MW + NEWENGLAND_GEO_MW")
                print(f"  2. NEWENGLAND_BATTERIES_MW = NEWENGLAND_BAT_MW + NEWENGLAND_PS_MW + NEWENGLAND_SNB_MW")
                print(f"  3. NEWENGLAND_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Newengland Data Wrangling Process =====")
    print(f"Input file: Newengland_complete_with_interchange.parquet")
    print(f"Output file: Newengland_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[NewenglandDataWrangler] = None

    try:
        # Instantiate the Newengland data wrangler
        wrangler = NewenglandDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Newengland Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Newengland Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Newengland Data Wrangling Process Complete ---")
    print("="*60)