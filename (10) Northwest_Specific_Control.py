...
DATE_COL = "date"
REGION = "northwest"
REGION_UPPER = "NORTHWEST"

# --- NORTHWESTDataProcessor Class Definition ---
class NORTHWESTDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NORTHWESTDataProcessor using Parquet files.
    ...
    def fill_NORTHWEST_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill NORTHWEST-specific data requirements:
        1. NORTHWEST_BAT_MW, NORTHWEST_SNB_MW: zeros until 2025-03-17 22:00
        2. NORTHWEST_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying NORTHWEST-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-01-14 22:00:00')

        # *** MODIFIED: Removed NORTHWEST_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['NORTHWEST_BAT_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'NORTHWEST_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for NORTHWEST_GEO_MW ***

        print("NORTHWEST-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the NORTHWEST data processing."""
        print(f"\n--- Processing NORTHWEST Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply NORTHWEST-specific data filling
            df_filled = self.fill_NORTHWEST_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting NORTHWEST-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if NORTHWEST is in the regions config
    if REGION not in regions:
        print(f"NORTHWEST not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing NORTHWEST region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[NORTHWESTDataProcessor] = None

    try:
        # Instantiate the NORTHWEST-specific processor
        processor = NORTHWESTDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"NORTHWEST (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"NORTHWEST (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- NORTHWEST Data Processing Complete ---")
    print("="*60)