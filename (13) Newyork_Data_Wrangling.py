...
# --- NewyorkDataWrangler Class Definition ---
class NewyorkDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NewyorkDataWrangler.
        ...
        self.mw_combinations = {
            "NEWYORK_OTH_MW": ["NEWYORK_OIL_MW", "NEWYORK_OTH_MW"],
        }
        
        self.capacity_combinations = {
            "Newyork_Total_Capacity_Other": [
                "Newyork_Total_Capacity_Other", 
                "Newyork_Total_Capacity_Oil",
                "Newyork_Total_Capacity_Pumped_Storage",
                "Newyork_Total_Capacity_Biomass",
                "Newyork_Total_Capacity_Batteries" 
    ...
    def run(self):
        """Execute the Newyork data wrangling process."""
        print(f"\n--- Starting Newyork Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. NEWYORK_OTH_MW = NEWYORK_OIL_MW + NEWYORK_OTH_MW + NEWYORK_GEO_MW")
                print(f"  2. NEWYORK_BATTERIES_MW = NEWYORK_BAT_MW + NEWYORK_PS_MW + NEWYORK_SNB_MW")
                print(f"  3. NEWYORK_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Newyork Data Wrangling Process =====")
    print(f"Input file: Newyork_complete_with_interchange.parquet")
    print(f"Output file: Newyork_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[NewyorkDataWrangler] = None

    try:
        # Instantiate the Newyork data wrangler
        wrangler = NewyorkDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Newyork Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Newyork Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Newyork Data Wrangling Process Complete ---")
    print("="*60)