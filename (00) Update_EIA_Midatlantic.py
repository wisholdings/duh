...
# Common fuel types - FIXED: Removed UES which was causing issues
fuel_types = {
    "NG": "natural_gas",
    "WND": "wind",
    "SUN": "solar",
...
# List of region configs - FIXED: Changed to MIDATLANTIC
region_configs = [
    {
    ...
    # Add more states or regions here if needed
]
...
def sql_batch_upsert_to_database(engine, table_name, data, chunk_size=25000):
    ...
    # If no primary keys found, default to DATETIME
    if not primary_keys:
        print(f"[WARNING] No primary keys found for {table_name}, defaulting to DATETIME")
    ...
    for i in range(0, len(data), chunk_size):
        ...
        # Ensure we have valid primary keys for the ON clause
        on_conditions = []
        update_conditions = []
        ...
        for col in columns:
            if col not in primary_keys:
                update_conditions.append(f"target.{col} = source.{col}")

        # Build the MERGE query with proper error handling
        if not on_conditions:
            print(f"[ERROR] No valid primary key conditions found for {table_name}")
            continue
        ...
        WHEN MATCHED THEN
            UPDATE SET {", ".join(update_conditions) if update_conditions else "target.DATETIME = source.DATETIME"}
...
def process_combined_data(combined_df, r_name, desired_end_time_local):
    ...
    # Ensure all expected columns exist (8 fuel types for Mid-Atlantic)
