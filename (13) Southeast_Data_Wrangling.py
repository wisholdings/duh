...
# --- SoutheastDataWrangler Class Definition ---
class SoutheastDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the SoutheastDataWrangler.
        ...
        self.mw_combinations = {
            "SOUTHEAST_OTH_MW": ["SOUTHEAST_OIL_MW", "SOUTHEAST_OTH_MW"],
            "SOUTHEAST_BAT_MW": ["SOUTHEAST_BAT_MW", "SOUTHEAST_SNB_MW","SOUTHEAST_OES_MW","SOUTHEAST_PS_MW"],

        }
        
        self.capacity_combinations = {
            "Southeast_Total_Capacity_Other": [
                "Southeast_Total_Capacity_Other", 
                "Southeast_Total_Capacity_Oil", 
                "Southeast_Total_Capacity_Biomass", 

            ],
            "Southeast_Total_Capacity_Batteries": [
                "Southeast_Total_Capacity_Batteries", 
                "Southeast_Total_Capacity_Pumped_Storage"
    ...
    def run(self):
        """Execute the Southeast data wrangling process."""
        print(f"\n--- Starting Southeast Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. SOUTHEAST_OTH_MW = SOUTHEAST_OIL_MW + SOUTHEAST_OTH_MW + SOUTHEAST_GEO_MW")
                print(f"  2. SOUTHEAST_BATTERIES_MW = SOUTHEAST_BAT_MW + SOUTHEAST_PS_MW + SOUTHEAST_SNB_MW")
                print(f"  3. SOUTHEAST_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Southeast Data Wrangling Process =====")
    print(f"Input file: Southeast_complete_with_interchange.parquet")
    print(f"Output file: Southeast_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[SoutheastDataWrangler] = None

    try:
        # Instantiate the Southeast data wrangler
        wrangler = SoutheastDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Southeast Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Southeast Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Southeast Data Wrangling Process Complete ---")
    print("="*60)