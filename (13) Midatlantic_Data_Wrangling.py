...
# --- MidatlanticDataWrangler Class Definition ---
class MidatlanticDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the MidatlanticDataWrangler.
        ...
        self.mw_combinations = {
            "MIDATLANTIC_OTH_MW": ["MIDATLANTIC_OIL_MW", "MIDATLANTIC_OTH_MW"],
        }
        
        self.capacity_combinations = {
            "Midatlantic_Total_Capacity_Other": [
                "Midatlantic_Total_Capacity_Other", 
                "Midatlantic_Total_Capacity_Oil",
                "Midatlantic_Total_Capacity_Biomass",
                "Midatlantic_Total_Capacity_Batteries", 
                "Midatlantic_Total_Capacity_Pumped_Storage",
                "Midatlantic_Total_Capacity_Solar_Batteries"
    ...
    def run(self):
        """Execute the Midatlantic data wrangling process."""
        print(f"\n--- Starting Midatlantic Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. MIDATLANTIC_OTH_MW = MIDATLANTIC_OIL_MW + MIDATLANTIC_OTH_MW + MIDATLANTIC_GEO_MW")
                print(f"  2. MIDATLANTIC_BATTERIES_MW = MIDATLANTIC_BAT_MW + MIDATLANTIC_PS_MW + MIDATLANTIC_SNB_MW")
                print(f"  3. MIDATLANTIC_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Midatlantic Data Wrangling Process =====")
    print(f"Input file: Midatlantic_complete_with_interchange.parquet")
    print(f"Output file: Midatlantic_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[MidatlanticDataWrangler] = None

    try:
        # Instantiate the Midatlantic data wrangler
        wrangler = MidatlanticDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Midatlantic Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Midatlantic Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Midatlantic Data Wrangling Process Complete ---")
    print("="*60)