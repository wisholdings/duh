...
DATE_COL = "date"
REGION = "central"
REGION_UPPER = "CENTRAL"

# --- CENTRALDataProcessor Class Definition ---
class CENTRALDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CENTRALDataProcessor using Parquet files.
    ...
    def fill_CENTRAL_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill CENTRAL-specific data requirements:
        1. CENTRAL_BAT_MW, CENTRAL_SNB_MW: zeros until 2025-03-17 22:00
        2. CENTRAL_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying CENTRAL-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-03-17 22:00:00')

        # *** MODIFIED: Removed CENTRAL_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['CENTRAL_BAT_MW', 'CENTRAL_SNB_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'CENTRAL_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for CENTRAL_GEO_MW ***

        print("CENTRAL-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the CENTRAL data processing."""
        print(f"\n--- Processing CENTRAL Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply CENTRAL-specific data filling
            df_filled = self.fill_CENTRAL_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting CENTRAL-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if CENTRAL is in the regions config
    if REGION not in regions:
        print(f"CENTRAL not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing CENTRAL region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[CENTRALDataProcessor] = None

    try:
        # Instantiate the CENTRAL-specific processor
        processor = CENTRALDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"CENTRAL (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"CENTRAL (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- CENTRAL Data Processing Complete ---")
    print("="*60)