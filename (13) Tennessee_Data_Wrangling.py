...
# --- TennesseeDataWrangler Class Definition ---
class TennesseeDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the TennesseeDataWrangler.
        ...
        self.mw_combinations = {
            "TENNESSEE_OTH_MW": ["TENNESSEE_OIL_MW", "TENNESSEE_OTH_MW","TENNESSEE_GEO_MW","TENNESSEE_OES_MW","TENNESSEE_BAT_MW"],
            "TENNESSEE_SUN_MW": ["TENNESSEE_SUN_MW", "TENNESSEE_SNB_MW"],

        }
        
        self.capacity_combinations = {
            "Tennessee_Total_Capacity_Other": [
                "Tennessee_Total_Capacity_Other", 
                "Tennessee_Total_Capacity_Oil", 
                "Tennessee_Total_Capacity_Biomass",
                "Tennessee_Total_Capacity_Batteries"
    ...
    def run(self):
        """Execute the Tennessee data wrangling process."""
        print(f"\n--- Starting Tennessee Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. TENNESSEE_OTH_MW = TENNESSEE_OIL_MW + TENNESSEE_OTH_MW + TENNESSEE_GEO_MW")
                print(f"  2. TENNESSEE_BATTERIES_MW = TENNESSEE_BAT_MW + TENNESSEE_PS_MW + TENNESSEE_SNB_MW")
                print(f"  3. TENNESSEE_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Tennessee Data Wrangling Process =====")
    print(f"Input file: Tennessee_complete_with_interchange.parquet")
    print(f"Output file: Tennessee_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[TennesseeDataWrangler] = None

    try:
        # Instantiate the Tennessee data wrangler
        wrangler = TennesseeDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Tennessee Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Tennessee Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Tennessee Data Wrangling Process Complete ---")
    print("="*60)