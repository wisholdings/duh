...
# --- MidwestDataCleaner Class Definition ---
class MidwestDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the MidwestDataCleaner.
    ...
    def forward_fill_non_Midwest_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'MIDWEST'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Midwest columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'MIDWEST' (excluding the date column)
        non_Midwest_cols = [col for col in df_out.columns 
                              if not col.startswith('MIDWEST') and col != self.date_col]
        
        print(f"Found {len(non_Midwest_cols)} non-Midwest columns to process:")
        for col in non_Midwest_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Midwest_cols) > 10:
            print(f"  ... and {len(non_Midwest_cols) - 10} more columns")

        if not non_Midwest_cols:
            print("  No non-Midwest columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Midwest column
        for col in non_Midwest_cols:
    ...
    def run(self):
        """Execute the Midwest data cleaning process."""
        print(f"\n--- Starting Midwest Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Midwest columns
            df_filled = self.forward_fill_non_Midwest_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Midwest Data Cleaning Process =====")
    print(f"Input file: Midwest_complete_finished.parquet")
    print(f"Output file: Midwest_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[MidwestDataCleaner] = None

    try:
        # Instantiate the Midwest data cleaner
        cleaner = MidwestDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Midwest Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Midwest Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Midwest Data Cleaning Process Complete ---")
    print("="*60)