...
# --- NewyorkDataCleaner Class Definition ---
class NewyorkDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NewyorkDataCleaner.
    ...
    def forward_fill_non_Newyork_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'NEWYORK'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Newyork columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'NEWYORK' (excluding the date column)
        non_Newyork_cols = [col for col in df_out.columns 
                              if not col.startswith('NEWYORK') and col != self.date_col]
        
        print(f"Found {len(non_Newyork_cols)} non-Newyork columns to process:")
        for col in non_Newyork_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Newyork_cols) > 10:
            print(f"  ... and {len(non_Newyork_cols) - 10} more columns")

        if not non_Newyork_cols:
            print("  No non-Newyork columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Newyork column
        for col in non_Newyork_cols:
    ...
    def run(self):
        """Execute the Newyork data cleaning process."""
        print(f"\n--- Starting Newyork Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Newyork columns
            df_filled = self.forward_fill_non_Newyork_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Newyork Data Cleaning Process =====")
    print(f"Input file: Newyork_complete_finished.parquet")
    print(f"Output file: Newyork_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[NewyorkDataCleaner] = None

    try:
        # Instantiate the Newyork data cleaner
        cleaner = NewyorkDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Newyork Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Newyork Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Newyork Data Cleaning Process Complete ---")
    print("="*60)