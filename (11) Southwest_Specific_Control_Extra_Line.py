...
# --- SouthwestDataCleaner Class Definition ---
class SouthwestDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the SouthwestDataCleaner.
    ...
    def forward_fill_non_Southwest_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'SOUTHWEST'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Southwest columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'SOUTHWEST' (excluding the date column)
        non_Southwest_cols = [col for col in df_out.columns 
                              if not col.startswith('SOUTHWEST') and col != self.date_col]
        
        print(f"Found {len(non_Southwest_cols)} non-Southwest columns to process:")
        for col in non_Southwest_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Southwest_cols) > 10:
            print(f"  ... and {len(non_Southwest_cols) - 10} more columns")

        if not non_Southwest_cols:
            print("  No non-Southwest columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Southwest column
        for col in non_Southwest_cols:
    ...
    def run(self):
        """Execute the Southwest data cleaning process."""
        print(f"\n--- Starting Southwest Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Southwest columns
            df_filled = self.forward_fill_non_Southwest_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Southwest Data Cleaning Process =====")
    print(f"Input file: Southwest_complete_finished.parquet")
    print(f"Output file: Southwest_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[SouthwestDataCleaner] = None

    try:
        # Instantiate the Southwest data cleaner
        cleaner = SouthwestDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Southwest Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Southwest Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Southwest Data Cleaning Process Complete ---")
    print("="*60)