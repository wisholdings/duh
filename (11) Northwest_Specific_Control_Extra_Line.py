...
# --- NorthwestDataCleaner Class Definition ---
class NorthwestDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NorthwestDataCleaner.
    ...
    def forward_fill_non_Northwest_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'NORTHWEST'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Northwest columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'NORTHWEST' (excluding the date column)
        non_Northwest_cols = [col for col in df_out.columns 
                              if not col.startswith('NORTHWEST') and col != self.date_col]
        
        print(f"Found {len(non_Northwest_cols)} non-Northwest columns to process:")
        for col in non_Northwest_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Northwest_cols) > 10:
            print(f"  ... and {len(non_Northwest_cols) - 10} more columns")

        if not non_Northwest_cols:
            print("  No non-Northwest columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Northwest column
        for col in non_Northwest_cols:
    ...
    def run(self):
        """Execute the Northwest data cleaning process."""
        print(f"\n--- Starting Northwest Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Northwest columns
            df_filled = self.forward_fill_non_Northwest_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Northwest Data Cleaning Process =====")
    print(f"Input file: Northwest_complete_finished.parquet")
    print(f"Output file: Northwest_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[NorthwestDataCleaner] = None

    try:
        # Instantiate the Northwest data cleaner
        cleaner = NorthwestDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Northwest Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Northwest Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Northwest Data Cleaning Process Complete ---")
    print("="*60)