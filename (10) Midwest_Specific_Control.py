...
DATE_COL = "date"
REGION = "midwest"
REGION_UPPER = "MIDWEST"

# --- MIDWESTDataProcessor Class Definition ---
class MIDWESTDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the MIDWESTDataProcessor using Parquet files.
    ...
    def fill_MIDWEST_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill MIDWEST-specific data requirements:
        1. MIDWEST_BAT_MW, MIDWEST_SNB_MW: zeros until 2025-03-17 22:00
        2. MIDWEST_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying MIDWEST-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-01-14 22:00:00')

        # *** MODIFIED: Removed MIDWEST_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['MIDWEST_BAT_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'MIDWEST_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for MIDWEST_GEO_MW ***

        print("MIDWEST-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the MIDWEST data processing."""
        print(f"\n--- Processing MIDWEST Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply MIDWEST-specific data filling
            df_filled = self.fill_MIDWEST_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting MIDWEST-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if MIDWEST is in the regions config
    if REGION not in regions:
        print(f"MIDWEST not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing MIDWEST region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[MIDWESTDataProcessor] = None

    try:
        # Instantiate the MIDWEST-specific processor
        processor = MIDWESTDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"MIDWEST (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"MIDWEST (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- MIDWEST Data Processing Complete ---")
    print("="*60)