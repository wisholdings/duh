...
# --- TEXASDataProcessor Class Definition ---
class TEXASDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the TEXASDataProcessor using Parquet files.
    ...
    def fill_TEXAS_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill TEXAS-specific data requirements:
        1. TEXAS_BAT_MW, TEXAS_SNB_MW: zeros until 2025-03-17 22:00
        2. TEXAS_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying TEXAS-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-01-14 22:00:00')

        # *** MODIFIED: Removed TEXAS_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['TEXAS_BAT_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'TEXAS_BAT_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for TEXAS_GEO_MW ***

        print("TEXAS-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the TEXAS data processing."""
        ...
        try:
            ...
            # 2. Apply TEXAS-specific data filling
            df_filled = self.fill_TEXAS_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting TEXAS-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if TEXAS is in the regions config
    if REGION not in regions:
        print(f"TEXAS not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing TEXAS region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[TEXASDataProcessor] = None

    try:
        # Instantiate the TEXAS-specific processor
        processor = TEXASDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"TEXAS (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"TEXAS (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- TEXAS Data Processing Complete ---")
    print("="*60)