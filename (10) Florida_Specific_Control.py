...
DATE_COL = "date"
REGION = "florida"
REGION_UPPER = "FLORIDA"

# --- FLORIDADataProcessor Class Definition ---
class FLORIDADataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the FLORIDADataProcessor using Parquet files.
    ...
    def fill_FLORIDA_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill FLORIDA-specific data requirements:
        1. FLORIDA_BAT_MW, FLORIDA_SNB_MW: zeros until 2025-03-17 22:00
        2. FLORIDA_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying FLORIDA-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-03-17 22:00:00')

        # *** MODIFIED: Removed FLORIDA_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['FLORIDA_BAT_MW', 'FLORIDA_SNB_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'FLORIDA_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for FLORIDA_GEO_MW ***

        print("FLORIDA-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the FLORIDA data processing."""
        ...
        try:
            ...
            # 2. Apply FLORIDA-specific data filling
            df_filled = self.fill_FLORIDA_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting FLORIDA-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if FLORIDA is in the regions config
    if REGION not in regions:
        print(f"FLORIDA not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing FLORIDA region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[FLORIDADataProcessor] = None

    try:
        # Instantiate the FLORIDA-specific processor
        processor = FLORIDADataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"FLORIDA (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"FLORIDA (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- FLORIDA Data Processing Complete ---")
    print("="*60)