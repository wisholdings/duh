...
# --- CentralDataCleaner Class Definition ---
class CentralDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CentralDataCleaner.
    ...
    def forward_fill_non_central_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'CENTRAL'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Central columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'CENTRAL' (excluding the date column)
        non_central_cols = [col for col in df_out.columns 
                              if not col.startswith('CENTRAL') and col != self.date_col]
        
        print(f"Found {len(non_central_cols)} non-Central columns to process:")
        for col in non_central_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_central_cols) > 10:
            print(f"  ... and {len(non_central_cols) - 10} more columns")

        if not non_central_cols:
            print("  No non-Central columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Central column
        for col in non_central_cols:
    ...
    def run(self):
        """Execute the Central data cleaning process."""
        print(f"\n--- Starting Central Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Central columns
            df_filled = self.forward_fill_non_central_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Central Data Cleaning Process =====")
    print(f"Input file: central_complete_finished.parquet")
    print(f"Output file: central_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[CentralDataCleaner] = None

    try:
        # Instantiate the Central data cleaner
        cleaner = CentralDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Central Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Central Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Central Data Cleaning Process Complete ---")
    print("="*60)