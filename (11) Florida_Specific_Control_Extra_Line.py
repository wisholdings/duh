...
# --- FloridaDataCleaner Class Definition ---
class FloridaDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the FloridaDataCleaner.
    ...
    def forward_fill_non_florida_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'FLORIDA'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Florida columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'FLORIDA' (excluding the date column)
        non_florida_cols = [col for col in df_out.columns 
                              if not col.startswith('FLORIDA') and col != self.date_col]
        
        print(f"Found {len(non_florida_cols)} non-Florida columns to process:")
        for col in non_florida_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_florida_cols) > 10:
            print(f"  ... and {len(non_florida_cols) - 10} more columns")

        if not non_florida_cols:
            print("  No non-Florida columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Florida column
        for col in non_florida_cols:
    ...
    def run(self):
        """Execute the Florida data cleaning process."""
        print(f"\n--- Starting Florida Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Florida columns
            df_filled = self.forward_fill_non_florida_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Florida Data Cleaning Process =====")
    print(f"Input file: florida_complete_finished.parquet")
    print(f"Output file: florida_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[FloridaDataCleaner] = None

    try:
        # Instantiate the Florida data cleaner
        cleaner = FloridaDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Florida Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Florida Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Florida Data Cleaning Process Complete ---")
    print("="*60)