...
# --- TexasDataCleaner Class Definition ---
class TexasDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the TexasDataCleaner.
    ...
    def forward_fill_non_Texas_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'TEXAS'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Texas columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'TEXAS' (excluding the date column)
        non_Texas_cols = [col for col in df_out.columns 
                              if not col.startswith('TEXAS') and col != self.date_col]
        
        print(f"Found {len(non_Texas_cols)} non-Texas columns to process:")
        for col in non_Texas_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Texas_cols) > 10:
            print(f"  ... and {len(non_Texas_cols) - 10} more columns")

        if not non_Texas_cols:
            print("  No non-Texas columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Texas column
        for col in non_Texas_cols:
    ...
    def run(self):
        """Execute the Texas data cleaning process."""
        print(f"\n--- Starting Texas Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Texas columns
            df_filled = self.forward_fill_non_Texas_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Texas Data Cleaning Process =====")
    print(f"Input file: Texas_complete_finished.parquet")
    print(f"Output file: Texas_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[TexasDataCleaner] = None

    try:
        # Instantiate the Texas data cleaner
        cleaner = TexasDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Texas Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Texas Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Texas Data Cleaning Process Complete ---")
    print("="*60)