...
# --- TexasDataWrangler Class Definition ---
class TexasDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the TexasDataWrangler.
        ...
        self.mw_combinations = {
            "TEXAS_OTH_MW": ["TEXAS_OIL_MW", "TEXAS_OTH_MW"],
            "TEXAS_BAT_MW": ["TEXAS_BAT_MW", "TEXAS_UES_MW"],

        }
        
        self.capacity_combinations = {
            "Texas_Total_Capacity_Other": [
                "Texas_Total_Capacity_Other", 
                "Texas_Total_Capacity_Oil", 
                "Texas_Total_Capacity_Biomass",
                "Texas_Total_Capacity_Geothermal"

            ],
            "Texas_Total_Capacity_Batteries": [
                "Texas_Total_Capacity_Batteries", 
                "Texas_Total_Capacity_Solar_Batteries", 
    ...
    def run(self):
        """Execute the Texas data wrangling process."""
        print(f"\n--- Starting Texas Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. TEXAS_OTH_MW = TEXAS_OIL_MW + TEXAS_OTH_MW + TEXAS_GEO_MW")
                print(f"  2. TEXAS_BATTERIES_MW = TEXAS_BAT_MW + TEXAS_PS_MW + TEXAS_SNB_MW")
                print(f"  3. TEXAS_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Texas Data Wrangling Process =====")
    print(f"Input file: Texas_complete_with_interchange.parquet")
    print(f"Output file: Texas_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[TexasDataWrangler] = None

    try:
        # Instantiate the Texas data wrangler
        wrangler = TexasDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Texas Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Texas Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Texas Data Wrangling Process Complete ---")
    print("="*60)