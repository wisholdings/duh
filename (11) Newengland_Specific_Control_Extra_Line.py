...
# --- NewenglandDataCleaner Class Definition ---
class NewenglandDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NewenglandDataCleaner.
    ...
    def forward_fill_non_Newengland_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'NEWENGLAND'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Newengland columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'NEWENGLAND' (excluding the date column)
        non_Newengland_cols = [col for col in df_out.columns 
                              if not col.startswith('NEWENGLAND') and col != self.date_col]
        
        print(f"Found {len(non_Newengland_cols)} non-Newengland columns to process:")
        for col in non_Newengland_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Newengland_cols) > 10:
            print(f"  ... and {len(non_Newengland_cols) - 10} more columns")

        if not non_Newengland_cols:
            print("  No non-Newengland columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Newengland column
        for col in non_Newengland_cols:
    ...
    def run(self):
        """Execute the Newengland data cleaning process."""
        print(f"\n--- Starting Newengland Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Newengland columns
            df_filled = self.forward_fill_non_Newengland_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Newengland Data Cleaning Process =====")
    print(f"Input file: Newengland_complete_finished.parquet")
    print(f"Output file: Newengland_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[NewenglandDataCleaner] = None

    try:
        # Instantiate the Newengland data cleaner
        cleaner = NewenglandDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Newengland Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Newengland Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Newengland Data Cleaning Process Complete ---")
    print("="*60)