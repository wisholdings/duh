...
DATE_COL = "date"
REGION = "carolina"
REGION_UPPER = "CAROLINA"

# --- CAROLINADataProcessor Class Definition ---
class CAROLINADataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CAROLINADataProcessor using Parquet files.
    ...
    def fill_CAROLINA_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill CAROLINA-specific data requirements:
        1. CAROLINA_BAT_MW, CAROLINA_SNB_MW: zeros until 2025-03-17 22:00
        2. CAROLINA_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying CAROLINA-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-03-17 23:00:00')

        # *** MODIFIED: Removed CAROLINA_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['CAROLINA_BAT_MW', 'CAROLINA_SNB_MW','CAROLINA_PS_MW']
        ...
        # *** MODIFIED: Removed the entire forward-fill section for CAROLINA_GEO_MW ***

        print("CAROLINA-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the CAROLINA data processing."""
        print(f"\n--- Processing CAROLINA Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply CAROLINA-specific data filling
            df_filled = self.fill_CAROLINA_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting CAROLINA-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if CAROLINA is in the regions config
    if REGION not in regions:
        print(f"CAROLINA not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing CAROLINA region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[CAROLINADataProcessor] = None

    try:
        # Instantiate the CAROLINA-specific processor
        processor = CAROLINADataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"CAROLINA (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"CAROLINA (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- CAROLINA Data Processing Complete ---")
    print("="*60)