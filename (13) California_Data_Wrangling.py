...
# --- CaliforniaDataWrangler Class Definition ---
class CaliforniaDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CaliforniaDataWrangler.
        ...
        self.mw_combinations = {
            "CALIFORNIA_OTH_MW": ["CALIFORNIA_OIL_MW", "CALIFORNIA_OTH_MW","CALIFORNIA_GEO_MW"],
            "CALIFORNIA_BAT_MW": ["CALIFORNIA_BAT_MW","CALIFORNIA_SNB_MW"]
        }
        
        self.capacity_combinations = {
            "California_Total_Capacity_Other": [
                "California_Total_Capacity_Other", 
                "California_Total_Capacity_Oil", 
                "California_Total_Capacity_Biomass",
                "California_Total_Capacity_Geothermal",
                "California_Total_Capacity_Solar_Batteries"
            ],
                        "California_Total_Capacity_Batteries": [

                "California_Total_Capacity_Batteries","California_Total_Capacity_Solar_Batteries"
            ]
        }
    ...
    def run(self):
        """Execute the California data wrangling process."""
        print(f"\n--- Starting California Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. CALIFORNIA_OTH_MW = CALIFORNIA_OIL_MW + CALIFORNIA_OTH_MW + CALIFORNIA_GEO_MW")
                print(f"  2. CALIFORNIA_BATTERIES_MW = CALIFORNIA_BAT_MW + CALIFORNIA_PS_MW + CALIFORNIA_SNB_MW")
                print(f"  3. CALIFORNIA_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting California Data Wrangling Process =====")
    print(f"Input file: california_complete_with_interchange.parquet")
    print(f"Output file: california_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[CaliforniaDataWrangler] = None

    try:
        # Instantiate the California data wrangler
        wrangler = CaliforniaDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"California Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"California Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- California Data Wrangling Process Complete ---")
    print("="*60)