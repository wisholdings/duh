...
# --- CarolinaDataCleaner Class Definition ---
class CarolinaDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CarolinaDataCleaner.
    ...
    def forward_fill_non_carolina_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'CAROLINA'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Carolina columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'CAROLINA' (excluding the date column)
        non_carolina_cols = [col for col in df_out.columns 
                              if not col.startswith('CAROLINA') and col != self.date_col]
        
        print(f"Found {len(non_carolina_cols)} non-Carolina columns to process:")
        for col in non_carolina_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_carolina_cols) > 10:
            print(f"  ... and {len(non_carolina_cols) - 10} more columns")

        if not non_carolina_cols:
            print("  No non-Carolina columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Carolina column
        for col in non_carolina_cols:
    ...
    def run(self):
        """Execute the Carolina data cleaning process."""
        print(f"\n--- Starting Carolina Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Carolina columns
            df_filled = self.forward_fill_non_carolina_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Carolina Data Cleaning Process =====")
    print(f"Input file: carolina_complete_finished.parquet")
    print(f"Output file: carolina_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[CarolinaDataCleaner] = None

    try:
        # Instantiate the Carolina data cleaner
        cleaner = CarolinaDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Carolina Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Carolina Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Carolina Data Cleaning Process Complete ---")
    print("="*60)