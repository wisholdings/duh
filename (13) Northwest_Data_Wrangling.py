...
# --- NorthwestDataWrangler Class Definition ---
class NorthwestDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NorthwestDataWrangler.
        ...
        self.mw_combinations = {
            "NORTHWEST_OTH_MW": ["NORTHWEST_OIL_MW", "NORTHWEST_OTH_MW","NORTHWEST_GEO_MW"],
            "NORTHWEST_BAT_MW": ["NORTHWEST_BAT_MW", "NORTHWEST_SNB_MW"],

        }
        
        self.capacity_combinations = {
            "Northwest_Total_Capacity_Other": [
                "Northwest_Total_Capacity_Other", 
                "Northwest_Total_Capacity_Oil", 
            "Northwest_Total_Capacity_Geothermal"
            ],

                "Northwest_Total_Capacity_Hydroelectric":[
                "Northwest_Total_Capacity_Pumped_Storage", 
                "Northwest_Total_Capacity_Hydroelectric", 

            ],
                "Northwest_Total_Capacity_Batteries":[
                "Northwest_Total_Capacity_Batteries", 
                "Northwest_Total_Capacity_Solar_Batteries", 
    ...
    def run(self):
        """Execute the Northwest data wrangling process."""
        print(f"\n--- Starting Northwest Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. NORTHWEST_OTH_MW = NORTHWEST_OIL_MW + NORTHWEST_OTH_MW + NORTHWEST_GEO_MW")
                print(f"  2. NORTHWEST_BATTERIES_MW = NORTHWEST_BAT_MW + NORTHWEST_PS_MW + NORTHWEST_SNB_MW")
                print(f"  3. NORTHWEST_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Northwest Data Wrangling Process =====")
    print(f"Input file: Northwest_complete_with_interchange.parquet")
    print(f"Output file: Northwest_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[NorthwestDataWrangler] = None

    try:
        # Instantiate the Northwest data wrangler
        wrangler = NorthwestDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Northwest Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Northwest Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Northwest Data Wrangling Process Complete ---")
    print("="*60)