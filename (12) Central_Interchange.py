...
CENTRAL_COLUMN = "CENTRAL_INTERCHANGE_MW"
DATE_COL = "date"
...
# --- CentralInterchangeMerger Class Definition ---
class CentralInterchangeMerger:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CentralInterchangeMerger.
        ...
        self.central_column = CENTRAL_COLUMN
    ...
    def load_existing_parquet(self) -> pd.DataFrame:
        """Load the existing central_complete_finished.parquet file."""
    ...
    def download_central_interchange(self) -> pd.DataFrame:
        """Download Central interchange data from the database with date filtering."""
        if self.engine is None:
            print("❌ No database connection available.")
            return pd.DataFrame()

        try:
            print(f"Downloading Central interchange data from {self.table_name}...")
            print(f"Filtering data from {self.start_date_filter} onwards...")
            
            # SQL query to get Central data with date filter
            query = f"""
            SELECT 
                DATETIME,
                {self.central_column}
            FROM {self.table_name}
            WHERE {self.central_column} IS NOT NULL
            ...
            if df.empty:
                print("⚠️  No Central interchange data found in the database for the specified date range.")
                return df
            ...
            if not df[self.central_column].empty:
                stats = df[self.central_column].describe()
                ...
                non_null_count = df[self.central_column].notna().sum()
    ...
    def merge_data(self, parquet_df: pd.DataFrame, interchange_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if interchange_df.empty:
            ...
            parquet_df[self.central_column] = None
            return parquet_df
        ...
        try:
            ...
            interchange_col_stats = merged_df[self.central_column].describe()
            non_null_interchange = merged_df[self.central_column].notna().sum()
    ...
    def run(self):
        """Execute the Central interchange data merge process."""
        print(f"\n--- Starting Central Interchange Data Merge ---")
        ...
        try:
            ...
            # 3. Download Central Interchange Data
            interchange_df = self.download_central_interchange()
            ...
            if success:
                print(f"\n✅ Central interchange data merge completed successfully!")
            ...
            for df_var in ['parquet_df', 'interchange_df', 'merged_df']:
                if df_var in locals() and not locals()[df_var].empty:
                    del locals()[df_var]
            gc.collect()
            print(f"\n--- Central Interchange Merge Process Finished ---")

# --- Main Execution Block ---
if __name__ == "__main__":
    print("===== Starting Central Interchange Data Merge =====")
    print(f"Input Parquet: central_complete_finished.parquet")
    print(f"Database table: {TABLE_NAME}")
    print(f"Central column: {CENTRAL_COLUMN}")
    print(f"Date filter: From {START_DATE_FILTER} onwards")
    print(f"Output Parquet: central_complete_with_interchange.parquet")
    
    merger = None

    try:
        # Instantiate the merger
        merger = CentralInterchangeMerger(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the merge process
        merger.run()

    except ValueError as e:
        print(f"Central Interchange Merger (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Central Interchange Merger (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Central Interchange Data Merge Complete ---")
    print("="*60)