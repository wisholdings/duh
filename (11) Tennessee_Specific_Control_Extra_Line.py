...
# --- TennesseeDataCleaner Class Definition ---
class TennesseeDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the TennesseeDataCleaner.
    ...
    def forward_fill_non_Tennessee_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'TENNESSEE'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-Tennessee columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'TENNESSEE' (excluding the date column)
        non_Tennessee_cols = [col for col in df_out.columns 
                              if not col.startswith('TENNESSEE') and col != self.date_col]
        
        print(f"Found {len(non_Tennessee_cols)} non-Tennessee columns to process:")
        for col in non_Tennessee_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_Tennessee_cols) > 10:
            print(f"  ... and {len(non_Tennessee_cols) - 10} more columns")

        if not non_Tennessee_cols:
            print("  No non-Tennessee columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-Tennessee column
        for col in non_Tennessee_cols:
    ...
    def run(self):
        """Execute the Tennessee data cleaning process."""
        print(f"\n--- Starting Tennessee Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-Tennessee columns
            df_filled = self.forward_fill_non_Tennessee_columns(df)
...
if __name__ == "__main__":
    print("===== Starting Tennessee Data Cleaning Process =====")
    print(f"Input file: Tennessee_complete_finished.parquet")
    print(f"Output file: Tennessee_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[TennesseeDataCleaner] = None

    try:
        # Instantiate the Tennessee data cleaner
        cleaner = TennesseeDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"Tennessee Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Tennessee Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Tennessee Data Cleaning Process Complete ---")
    print("="*60)