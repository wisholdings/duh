...
SOUTHEAST_COLUMN = "SOUTHEAST_INTERCHANGE_MW"
DATE_COL = "date"
...
# --- SoutheastInterchangeMerger Class Definition ---
class SoutheastInterchangeMerger:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the SoutheastInterchangeMerger.
        ...
        self.Southeast_column = SOUTHEAST_COLUMN
    ...
    def load_existing_parquet(self) -> pd.DataFrame:
        """Load the existing Southeast_complete_finished.parquet file."""
    ...
    def download_Southeast_interchange(self) -> pd.DataFrame:
        """Download Southeast interchange data from the database with date filtering."""
        if self.engine is None:
            print("❌ No database connection available.")
            return pd.DataFrame()

        try:
            print(f"Downloading Southeast interchange data from {self.table_name}...")
            print(f"Filtering data from {self.start_date_filter} onwards...")
            
            # SQL query to get Southeast data with date filter
            query = f"""
            SELECT 
                DATETIME,
                {self.Southeast_column}
            FROM {self.table_name}
            WHERE {self.Southeast_column} IS NOT NULL
            ...
            if df.empty:
                print("⚠️  No Southeast interchange data found in the database for the specified date range.")
                return df
            ...
            if not df[self.Southeast_column].empty:
                stats = df[self.Southeast_column].describe()
                ...
                non_null_count = df[self.Southeast_column].notna().sum()
    ...
    def merge_data(self, parquet_df: pd.DataFrame, interchange_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if interchange_df.empty:
            ...
            parquet_df[self.Southeast_column] = None
            return parquet_df
        ...
        try:
            ...
            interchange_col_stats = merged_df[self.Southeast_column].describe()
            non_null_interchange = merged_df[self.Southeast_column].notna().sum()
    ...
    def run(self):
        """Execute the Southeast interchange data merge process."""
        print(f"\n--- Starting Southeast Interchange Data Merge ---")
        ...
        try:
            ...
            # 3. Download Southeast Interchange Data
            interchange_df = self.download_Southeast_interchange()
            ...
            if success:
                print(f"\n✅ Southeast interchange data merge completed successfully!")
            ...
            for df_var in ['parquet_df', 'interchange_df', 'merged_df']:
                if df_var in locals() and not locals()[df_var].empty:
                    del locals()[df_var]
            gc.collect()
            print(f"\n--- Southeast Interchange Merge Process Finished ---")

# --- Main Execution Block ---
if __name__ == "__main__":
    print("===== Starting Southeast Interchange Data Merge =====")
    print(f"Input Parquet: Southeast_complete_finished.parquet")
    print(f"Database table: {TABLE_NAME}")
    print(f"Southeast column: {SOUTHEAST_COLUMN}")
    print(f"Date filter: From {START_DATE_FILTER} onwards")
    print(f"Output Parquet: Southeast_complete_with_interchange.parquet")
    
    merger = None

    try:
        # Instantiate the merger
        merger = SoutheastInterchangeMerger(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the merge process
        merger.run()

    except ValueError as e:
        print(f"Southeast Interchange Merger (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Southeast Interchange Merger (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Southeast Interchange Data Merge Complete ---")
    print("="*60)