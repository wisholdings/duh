...
# --- CarolinaDataWrangler Class Definition ---
class CarolinaDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CarolinaDataWrangler.
        ...
        self.mw_combinations = {
            "CAROLINA_OTH_MW": ["CAROLINA_OIL_MW", "CAROLINA_OTH_MW", "CAROLINA_GEO_MW"],
            "CAROLINA_BAT_MW": ["CAROLINA_BAT_MW", "CAROLINA_SNB_MW"],

        }
        
        self.capacity_combinations = {
            "Carolina_Total_Capacity_Other": [
                "Carolina_Total_Capacity_Other", 
                "Carolina_Total_Capacity_Oil", 
                "Carolina_Total_Capacity_Geothermal",
                "Carolina_Total_Capacity_Biomass"
            ]
        }
    ...
    def run(self):
        """Execute the Carolina data wrangling process."""
        print(f"\n--- Starting Carolina Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. CAROLINA_OTH_MW = CAROLINA_OIL_MW + CAROLINA_OTH_MW + CAROLINA_GEO_MW")
                print(f"  2. CAROLINA_BATTERIES_MW = CAROLINA_BAT_MW + CAROLINA_PS_MW + CAROLINA_SNB_MW")
                print(f"  3. CAROLINA_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Carolina Data Wrangling Process =====")
    print(f"Input file: carolina_complete_with_interchange.parquet")
    print(f"Output file: carolina_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[CarolinaDataWrangler] = None

    try:
        # Instantiate the Carolina data wrangler
        wrangler = CarolinaDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Carolina Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Carolina Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Carolina Data Wrangling Process Complete ---")
    print("="*60)