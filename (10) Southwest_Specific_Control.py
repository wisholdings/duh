...
DATE_COL = "date"
REGION = "southwest"
REGION_UPPER = "SOUTHWEST"

# --- SOUTHWESTDataProcessor Class Definition ---
class SOUTHWESTDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the SOUTHWESTDataProcessor using Parquet files.
    ...
    def fill_SOUTHWEST_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill SOUTHWEST-specific data requirements:
        1. SOUTHWEST_BAT_MW, SOUTHWEST_SNB_MW: zeros until 2025-03-17 22:00
        2. SOUTHWEST_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying SOUTHWEST-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-01-14 22:00:00')

        # *** MODIFIED: Removed SOUTHWEST_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['SOUTHWEST_BAT_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'SOUTHWEST_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for SOUTHWEST_GEO_MW ***

        print("SOUTHWEST-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the SOUTHWEST data processing."""
        print(f"\n--- Processing SOUTHWEST Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply SOUTHWEST-specific data filling
            df_filled = self.fill_SOUTHWEST_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting SOUTHWEST-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if SOUTHWEST is in the regions config
    if REGION not in regions:
        print(f"SOUTHWEST not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing SOUTHWEST region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[SOUTHWESTDataProcessor] = None

    try:
        # Instantiate the SOUTHWEST-specific processor
        processor = SOUTHWESTDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"SOUTHWEST (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"SOUTHWEST (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- SOUTHWEST Data Processing Complete ---")
    print("="*60)