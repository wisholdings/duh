...
MIDWEST_COLUMN = "MIDWEST_INTERCHANGE_MW"
DATE_COL = "date"
...
# --- MidwestInterchangeMerger Class Definition ---
class MidwestInterchangeMerger:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the MidwestInterchangeMerger.
        ...
        self.Midwest_column = MIDWEST_COLUMN
    ...
    def load_existing_parquet(self) -> pd.DataFrame:
        """Load the existing Midwest_complete_finished.parquet file."""
    ...
    def download_Midwest_interchange(self) -> pd.DataFrame:
        """Download Midwest interchange data from the database with date filtering."""
        if self.engine is None:
            print("❌ No database connection available.")
            return pd.DataFrame()

        try:
            print(f"Downloading Midwest interchange data from {self.table_name}...")
            print(f"Filtering data from {self.start_date_filter} onwards...")
            
            # SQL query to get Midwest data with date filter
            query = f"""
            SELECT 
                DATETIME,
                {self.Midwest_column}
            FROM {self.table_name}
            WHERE {self.Midwest_column} IS NOT NULL
            ...
            if df.empty:
                print("⚠️  No Midwest interchange data found in the database for the specified date range.")
                return df
            ...
            if not df[self.Midwest_column].empty:
                stats = df[self.Midwest_column].describe()
                ...
                non_null_count = df[self.Midwest_column].notna().sum()
    ...
    def merge_data(self, parquet_df: pd.DataFrame, interchange_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if interchange_df.empty:
            ...
            parquet_df[self.Midwest_column] = None
            return parquet_df
        ...
        try:
            ...
            interchange_col_stats = merged_df[self.Midwest_column].describe()
            non_null_interchange = merged_df[self.Midwest_column].notna().sum()
    ...
    def run(self):
        """Execute the Midwest interchange data merge process."""
        print(f"\n--- Starting Midwest Interchange Data Merge ---")
        ...
        try:
            ...
            # 3. Download Midwest Interchange Data
            interchange_df = self.download_Midwest_interchange()
            ...
            if success:
                print(f"\n✅ Midwest interchange data merge completed successfully!")
            ...
            for df_var in ['parquet_df', 'interchange_df', 'merged_df']:
                if df_var in locals() and not locals()[df_var].empty:
                    del locals()[df_var]
            gc.collect()
            print(f"\n--- Midwest Interchange Merge Process Finished ---")

# --- Main Execution Block ---
if __name__ == "__main__":
    print("===== Starting Midwest Interchange Data Merge =====")
    print(f"Input Parquet: Midwest_complete_finished.parquet")
    print(f"Database table: {TABLE_NAME}")
    print(f"Midwest column: {MIDWEST_COLUMN}")
    print(f"Date filter: From {START_DATE_FILTER} onwards")
    print(f"Output Parquet: Midwest_complete_with_interchange.parquet")
    
    merger = None

    try:
        # Instantiate the merger
        merger = MidwestInterchangeMerger(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the merge process
        merger.run()

    except ValueError as e:
        print(f"Midwest Interchange Merger (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Midwest Interchange Merger (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Midwest Interchange Data Merge Complete ---")
    print("="*60)