...
DATE_COL = "date"
REGION = "midatlantic"
REGION_UPPER = "MIDATLANTIC"

# --- MIDATLANTICDataProcessor Class Definition ---
class MIDATLANTICDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the MIDATLANTICDataProcessor using Parquet files.
    ...
    def fill_MIDATLANTIC_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill MIDATLANTIC-specific data requirements:
        1. MIDATLANTIC_BAT_MW, MIDATLANTIC_SNB_MW: zeros until 2025-03-17 22:00
        2. MIDATLANTIC_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying MIDATLANTIC-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-03-17 22:00:00')

        # *** MODIFIED: Removed MIDATLANTIC_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['MIDATLANTIC_BAT_MW', 'MIDATLANTIC_SNB_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'MIDATLANTIC_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for MIDATLANTIC_GEO_MW ***

        print("MIDATLANTIC-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the MIDATLANTIC data processing."""
        print(f"\n--- Processing MIDATLANTIC Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply MIDATLANTIC-specific data filling
            df_filled = self.fill_MIDATLANTIC_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting MIDATLANTIC-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if MIDATLANTIC is in the regions config
    if REGION not in regions:
        print(f"MIDATLANTIC not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing MIDATLANTIC region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[MIDATLANTICDataProcessor] = None

    try:
        # Instantiate the MIDATLANTIC-specific processor
        processor = MIDATLANTICDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"MIDATLANTIC (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"MIDATLANTIC (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- MIDATLANTIC Data Processing Complete ---")
    print("="*60)