...
DATE_COL = "date"
REGION = "newyork"
REGION_UPPER = "NEWYORK"

# --- NEWYORKDataProcessor Class Definition ---
class NEWYORKDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the NEWYORKDataProcessor using Parquet files.
    ...
    def fill_NEWYORK_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill NEWYORK-specific data requirements:
        1. NEWYORK_BAT_MW, NEWYORK_SNB_MW: zeros until 2025-03-17 22:00
        2. NEWYORK_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying NEWYORK-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-01-14 22:00:00')

        # *** MODIFIED: Removed NEWYORK_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['NEWYORK_BAT_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'NEWYORK_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for NEWYORK_GEO_MW ***

        print("NEWYORK-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the NEWYORK data processing."""
        print(f"\n--- Processing NEWYORK Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply NEWYORK-specific data filling
            df_filled = self.fill_NEWYORK_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting NEWYORK-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if NEWYORK is in the regions config
    if REGION not in regions:
        print(f"NEWYORK not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing NEWYORK region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[NEWYORKDataProcessor] = None

    try:
        # Instantiate the NEWYORK-specific processor
        processor = NEWYORKDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"NEWYORK (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"NEWYORK (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- NEWYORK Data Processing Complete ---")
    print("="*60)