...
# --- CaliforniaDataCleaner Class Definition ---
class CaliforniaDataCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CaliforniaDataCleaner.
    ...
    def forward_fill_non_california_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Forward fill missing data in columns that DON'T start with 'CALIFORNIA'.
        """
        if df is None or df.empty:
            print("Skipping forward fill for empty or None DataFrame.")
            return df

        print("\nApplying forward fill to non-California columns...")
        df_out = df.copy()

        # Identify columns that don't start with 'CALIFORNIA' (excluding the date column)
        non_california_cols = [col for col in df_out.columns 
                              if not col.startswith('CALIFORNIA') and col != self.date_col]
        
        print(f"Found {len(non_california_cols)} non-California columns to process:")
        for col in non_california_cols[:10]:  # Show first 10 for brevity
            print(f"  - {col}")
        if len(non_california_cols) > 10:
            print(f"  ... and {len(non_california_cols) - 10} more columns")

        if not non_california_cols:
            print("  No non-California columns found to forward fill.")
            return df_out

        # Apply forward fill to each non-California column
        for col in non_california_cols:
    ...
    def run(self):
        """Execute the California data cleaning process."""
        print(f"\n--- Starting California Data Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Forward fill non-California columns
            df_filled = self.forward_fill_non_california_columns(df)
...
if __name__ == "__main__":
    print("===== Starting California Data Cleaning Process =====")
    print(f"Input file: california_complete_finished.parquet")
    print(f"Output file: california_complete_cleaned.parquet")
    print(f"Cutoff date: {CUTOFF_DATE}")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[CaliforniaDataCleaner] = None

    try:
        # Instantiate the California data cleaner
        cleaner = CaliforniaDataCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"California Data Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"California Data Cleaner (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- California Data Cleaning Process Complete ---")
    print("="*60)