...
DATE_COL = "date"
REGION = "southeast"
REGION_UPPER = "SOUTHEAST"

# --- SOUTHEASTDataProcessor Class Definition ---
class SOUTHEASTDataProcessor:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the SOUTHEASTDataProcessor using Parquet files.
    ...
    def fill_SOUTHEAST_specific_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Fill SOUTHEAST-specific data requirements:
        1. SOUTHEAST_BAT_MW, SOUTHEAST_SNB_MW: zeros until 2025-03-17 22:00
        2. SOUTHEAST_PS_MW: zeros until 2025-03-17 22:00
        """
        if df is None or df.empty:
            print("Skipping data filling for empty or None DataFrame.")
            return df

        print("\nApplying SOUTHEAST-specific data filling...")
        df_out = df.copy()

        # Define cutoff dates
        cutoff_2025_03_17 = pd.to_datetime('2025-01-14 22:00:00')

        # *** MODIFIED: Removed SOUTHEAST_GEO_MW from this list ***
        # Columns to fill with zeros until the cutoff
        early_columns = ['SOUTHEAST_BAT_MW']
        
        # Column to fill with zeros until the cutoff
        late_column = 'SOUTHEAST_PS_MW'
        ...
        # *** MODIFIED: Removed the entire forward-fill section for SOUTHEAST_GEO_MW ***

        print("SOUTHEAST-specific data filling complete.")
        return df_out
    ...
    def run(self):
        """Execute the SOUTHEAST data processing."""
        print(f"\n--- Processing SOUTHEAST Data ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            # 2. Apply SOUTHEAST-specific data filling
            df_filled = self.fill_SOUTHEAST_specific_data(df)
...
if __name__ == "__main__":
    print("===== Starting SOUTHEAST-Specific Data Processing =====")

    if not regions:
        print("No regions loaded from config/regions.json. Nothing to process.")
        sys.exit(0)

    # Check if SOUTHEAST is in the regions config
    if REGION not in regions:
        print(f"SOUTHEAST not found in regions config. Available regions: {list(regions.keys())}")
        sys.exit(1)

    print(f"Processing SOUTHEAST region specifically")
    print(f"Using Base Output Directory for data: {BASE_OUTPUT_DIR}")

    processor: Optional[SOUTHEASTDataProcessor] = None

    try:
        # Instantiate the SOUTHEAST-specific processor
        processor = SOUTHEASTDataProcessor(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data processing
        processor.run()

    except ValueError as e:
        print(f"SOUTHEAST (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"SOUTHEAST (Critical Unhandled Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- SOUTHEAST Data Processing Complete ---")
    print("="*60)