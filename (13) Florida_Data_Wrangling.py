...
# --- FloridaDataWrangler Class Definition ---
class FloridaDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the FloridaDataWrangler.
        ...
        self.mw_combinations = {
            "FLORIDA_OTH_MW": ["FLORIDA_OIL_MW", "FLORIDA_OTH_MW", "FLORIDA_GEO_MW"],
            "FLORIDA_BATTERIES_MW": ["FLORIDA_BAT_MW", "FLORIDA_PS_MW", "FLORIDA_SNB_MW"]
        }
        
        self.capacity_combinations = {
            "Florida_Total_Capacity_Other": [
                "Florida_Total_Capacity_Other", 
                "Florida_Total_Capacity_Oil", 
                "Florida_Total_Capacity_Geothermal",
                "Florida_Total_Capacity_Biomass"
            ],

           "Florida_Total_Capacity_Batteries": ["Florida_Total_Capacity_Batteries","Florida_Total_Capacity_Solar_Batteries"]
        }
    ...
    def run(self):
        """Execute the Florida data wrangling process."""
        print(f"\n--- Starting Florida Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. FLORIDA_OTH_MW = FLORIDA_OIL_MW + FLORIDA_OTH_MW + FLORIDA_GEO_MW")
                print(f"  2. FLORIDA_BATTERIES_MW = FLORIDA_BAT_MW + FLORIDA_PS_MW + FLORIDA_SNB_MW")
                print(f"  3. FLORIDA_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Florida Data Wrangling Process =====")
    print(f"Input file: florida_complete_with_interchange.parquet")
    print(f"Output file: florida_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[FloridaDataWrangler] = None

    try:
        # Instantiate the Florida data wrangler
        wrangler = FloridaDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Florida Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Florida Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Florida Data Wrangling Process Complete ---")
    print("="*60)