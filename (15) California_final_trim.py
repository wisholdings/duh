...
# --- CaliforniaBackfillCleaner Class Definition ---
class CaliforniaBackfillCleaner:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the CaliforniaBackfillCleaner.
        ...
        self.oth_columns = ["CALIFORNIA_OTH_RATIO", "CALIFORNIA_OTH_MW"]
    ...
    def clean_oth_data_after_reference(self, df: pd.DataFrame) -> pd.DataFrame:
        """Clean CALIFORNIA_OTH_RATIO and CALIFORNIA_OTH_MW data after reference date."""
        if df is None or df.empty:
    ...
    def run(self):
        """Execute the California backfill and cleaning process."""
        print(f"\n--- Starting California Backfill and OTH Cleaning ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                print(f"\n--- California backfill and cleaning completed successfully ---")
            ...
            for df_var in ['df', 'df_backfilled', 'df_cleaned']:
                if df_var in locals() and locals()[df_var] is not None:
                    del locals()[df_var]
            gc.collect()
            print(f"\n--- California Backfill Cleaner finished ---")

# --- Main Execution Block ---
if __name__ == "__main__":
    print("===== Starting California Backfill and OTH Cleaning Process =====")
    ...
    print(f"Output file: california_complete_backfilled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    cleaner: Optional[CaliforniaBackfillCleaner] = None

    try:
        # Instantiate the cleaner
        cleaner = CaliforniaBackfillCleaner(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the cleaning process
        cleaner.run()

    except ValueError as e:
        print(f"California Backfill Cleaner (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"California Backfill Cleaner (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- California Backfill and OTH Cleaning Complete ---")
    print("="*60)