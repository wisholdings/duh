...
# --- MidwestDataWrangler Class Definition ---
class MidwestDataWrangler:
    def __init__(self, base_dir: str = BASE_OUTPUT_DIR):
        """
        Initialize the MidwestDataWrangler.
        ...
        self.mw_combinations = {
            "MIDWEST_OTH_MW": ["MIDWEST_OIL_MW", "MIDWEST_OTH_MW"],
        }
        
        self.capacity_combinations = {
            "Midwest_Total_Capacity_Other": [
                "Midwest_Total_Capacity_Other", 
                "Midwest_Total_Capacity_Oil", 
                "Midwest_Total_Capacity_Biomass"

            ],
                        "Midwest_Total_Capacity_Batteries": [
                "Midwest_Total_Capacity_Solar_Batteries", 
                "Midwest_Total_Capacity_Pumped_Storage",
                "Midwest_Total_Capacity_Batteries" 
    ...
    def run(self):
        """Execute the Midwest data wrangling process."""
        print(f"\n--- Starting Midwest Data Wrangling ---")
        df: Optional[pd.DataFrame] = None
        ...
        try:
            ...
            if save_successful:
                ...
                print(f"  1. MIDWEST_OTH_MW = MIDWEST_OIL_MW + MIDWEST_OTH_MW + MIDWEST_GEO_MW")
                print(f"  2. MIDWEST_BATTERIES_MW = MIDWEST_BAT_MW + MIDWEST_PS_MW + MIDWEST_SNB_MW")
                print(f"  3. MIDWEST_Total_Capacity_Other = sum of Other + Oil + Geothermal capacities")
...
if __name__ == "__main__":
    print("===== Starting Midwest Data Wrangling Process =====")
    print(f"Input file: Midwest_complete_with_interchange.parquet")
    print(f"Output file: Midwest_complete_wrangled.parquet")
    print(f"Using Base Output Directory: {BASE_OUTPUT_DIR}")

    wrangler: Optional[MidwestDataWrangler] = None

    try:
        # Instantiate the Midwest data wrangler
        wrangler = MidwestDataWrangler(base_dir=BASE_OUTPUT_DIR)
        
        # Execute the data wrangling process
        wrangler.run()

    except ValueError as e:
        print(f"Midwest Data Wrangler (Config/Init Error): {e}")
        traceback.print_exc()
    except Exception as e:
        print(f"Midwest Data Wrangler (Critical Error): {e}")
        traceback.print_exc()
    finally:
    ...
    print("\n" + "="*60)
    print("--- Midwest Data Wrangling Process Complete ---")
    print("="*60)